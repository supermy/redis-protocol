...
/**
 *
 * 元数据与数据是两套metakey/metaval;datakey/dataval;由于value体积较大便于设置不同的缓存策略；
 *
 ...
 */
public class HyperLogLogMeta extends BaseMeta {
...
//    private ByteBuf metaVal;
    ...
    public static HyperLogLogMeta getInstance(RocksDB db0, byte[] ns0) {
        instance.db = db0;
        instance.NS = ns0;
        instance.VAlTYPE = DataType.KEY_HYPERLOGLOG;
        return instance;
    }
...
//    }
    ...
    public BulkReply get(byte[] key0) throws RedisException {
        if (checkTypeAndTTL(key0, DataType.KEY_HYPERLOGLOG)) return NIL_REPLY;
...
//    }
    ...
    public IntegerReply pfadd(byte[]... members) throws RedisException{
        ...
        if (checkTypeAndTTL(getKey0(), DataType.KEY_HYPERLOGLOG)) return integer(0);
...
//                }
        ...
        hyperLogLogCache.invalidate(MyUtils.ByteBuf2String(getMetaKey(key0[0])));
//
//        try {
...
//
//        buf.writeInt(DataType.KEY_HYPERLOGLOG); //value type
//        buf.writeBytes(DataType.SPLIT);
//
...
//                            db.put(getMetaKey2Byte(notification.getKey().array()), getMetaVal(hll.getBytes(), -1));
                            db.put(getDataKey2Byte(notification.getKey().array(),DataType.KEY_HYPERLOGLOG_DATA),MyUtils.toByteArray(genDataVal(hll.getBytes(), -1)));


                            notification.getKey().resetReaderIndex();
...
//                    byte[] value = db.get(getMetaKey2Byte(key0.array()));
                    byte[] value = db.get(getDataKey2Byte(key0.array(),DataType.KEY_HYPERLOGLOG_DATA));
