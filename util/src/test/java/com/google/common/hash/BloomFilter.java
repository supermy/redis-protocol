...
public final class BloomFilter<T> implements Predicate<T>, Serializable {
     ...
     * @throws IOException
     */
    public void writeToCompress(OutputStream out) throws IOException {
        // Serial form:
        // 1 signed byte for the strategy
        // 1 unsigned byte for the number of hash functions
        // 1 big endian int, the number of longs in our bitset
        ...
        Roaring64NavigableMap rr2 = Roaring64NavigableMap.bitmapOf();

        for (int i = 0; i < bits.data.length(); i++) {
            rr2.addLong(bits.data.get(i)); //利用Roaring64进行数据压缩
            dout.writeLong(bits.data.get(i));
        }


        {
            long[] longs = rr2.toArray();
            byte[] compress = Snappy.compress(longs);
            System.out.println("Roaring long 数组长度："+ MyUtils.bytes2kb(longs.length));
            System.out.println("Roaring 压缩后"+MyUtils.bytes2kb(compress.length));
        }
